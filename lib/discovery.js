...
// Domains that are never the brand's own site — articles, forums, directories,
// social platforms. If a search result comes from one of these, it might tell us
// a brand *exists*, but its URL is never usable as a product_url.
...
export async function tavilySearch(query) {
  ...
  // drop results whose own URL isn't a usable brand/retail site —
  // still useful as context for names/facts, but never as the product_url
...
{"products": [
  ...
  // final safety net: drop anything whose URL isn't exactly one we gave it
