...
// Reels: same container -> poll -> publish pattern as images, but media_type
// REELS + video_url, and video processing genuinely takes longer than images —
// give it a much longer window.
export async function publishReelToInstagram({ videoUrl, caption }) {
  ...
  const containerData = await containerRes.json();
  if (!containerRes.ok) {
    throw new Error(`Reel container creation failed: ${JSON.stringify(containerData)}`);
  }

  // video processing can genuinely take 30-90+ seconds
  await waitForContainerReady(containerData.id, accessToken, { timeoutMs: 50000, intervalMs: 3000 });
...
async function checkEligibility(supabase, product) {
...
export { checkEligibility, generateCaption as generateCaptionOnly };
